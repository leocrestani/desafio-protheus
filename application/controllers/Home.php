...
class Home extends CI_Controller
{
	...
	public function writeTxt()
	{
		...
		$strConstructor = str_pad('Simbolo', 10, ' ', STR_PAD_RIGHT) .
			str_pad('Nome', 30, ' ', STR_PAD_RIGHT) .
			str_pad('Cotação', 15, ' ', STR_PAD_RIGHT) .
			str_pad('Valor de Mercado', 25, ' ', STR_PAD_RIGHT) .
			str_pad('Volume de Transações', 25, ' ', STR_PAD_RIGHT) .
			str_pad('Moeda', 10, ' ', STR_PAD_RIGHT) . 'Data';
		fwrite($file, $strConstructor . PHP_EOL);
		foreach ($allCotations as $line) {
			$strConstructor = str_pad($line['simbolo'], 10, ' ', STR_PAD_RIGHT) .
				str_pad($line['nome'] ?: " ", 30, ' ', STR_PAD_RIGHT) .
				str_pad($line['cotacao'], 15, ' ', STR_PAD_RIGHT) .
				str_pad($line['valorMercado'] ?: "", 25, ' ', STR_PAD_RIGHT) .
				str_pad($line['volumeTransacoes'], 25, ' ', STR_PAD_RIGHT) .
				str_pad($line['moeda'], 10, ' ', STR_PAD_RIGHT) . $line['data'];
			fwrite($file, $strConstructor . PHP_EOL);
		}
		fclose($file);
	}
